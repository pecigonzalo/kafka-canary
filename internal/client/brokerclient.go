...
func NewBrokerAdminClient(
	...
	logger.Debug().Msg("Getting supported API versions")
	apiVersions, err := client.ApiVersions(ctx, &kafka.ApiVersionsRequest{})
	if err != nil {
		return nil, err
	}
	logger.Debug().Msgf("Supported API versions: %+v", apiVersions)
	...
	logger.Debug().Msgf("Supported features: %+v", supportedFeatures)
	...
	if config.ExpectedClusterID != "" {
		logger.Info().Msg("Checking cluster ID against version in cluster")
...
func (c *BrokerAdminClient) GetBrokers(ctx context.Context, ids []int) (
	...
	c.logger.Debug().Msgf("DescribeConfigs request: %+v", configsReq)

	configsResp, err := c.client.DescribeConfigs(ctx, &configsReq)
	c.logger.Debug().Msgf("DescribeConfigs response: %+v (%+v)", configsResp, err)
	if err != nil {
		return nil, err
	}
	...
	for t, topic := range metadataResp.Topics {
		if topic.Error != nil {
			if strings.Contains(topic.Error.Error(), "does not exist") {
				c.logger.Debug().Msgf("Skipping over topic %s because it does not exist", topic.Name)
				continue
			}
			// Some other error
			return nil,
...
func (c *BrokerAdminClient) UpdateTopicConfig(
	...
	c.logger.Debug().Msgf("IncrementalAlterConfigs request: %+v", req)

	// TODO: Handle case where overwrite is false.
	resp, err := c.client.IncrementalAlterConfigs(ctx, &req)
	c.logger.Debug().Msgf("IncrementalAlterConfigs response: %+v (%+v)", resp, err)
	if err != nil {
		return nil, err
	}
...
func (c *BrokerAdminClient) CreateTopic(
	...
	c.logger.Debug().Msgf("CreateTopics request: %+v", req)

	resp, err := c.client.CreateTopics(ctx, &req)
	c.logger.Debug().Msgf("CreateTopics response: %+v (%+v)", resp, err)
	if err != nil {
		return err
	}
...
func (c *BrokerAdminClient) AssignPartitions(
	...
	c.logger.Debug().Msgf("AlterPartitionReassignments request: %+v", req)

	resp, err := c.client.AlterPartitionReassignments(ctx, &req)
	c.logger.Debug().Msgf("AlterPartitionReassignments response: %+v (%+v)", resp, err)
	if err != nil {
		return err
	}
...
func (c *BrokerAdminClient) AddPartitions(
	...
	c.logger.Debug().Msgf("CreatePartitions request: %+v", req)

	resp, err := c.client.CreatePartitions(ctx, &req)
	c.logger.Debug().Msgf("CreatePartitions response: %+v (%+v)", resp, err)
	if err != nil {
		return err
	}
	if err = KafkaErrorsToErr(resp.Errors); err != nil {
...
func (c *BrokerAdminClient) RunLeaderElection(
	...
	c.logger.Debug().Msgf("ElectLeaders request: %+v", req)

	resp, err := c.client.ElectLeaders(ctx, &req)
	c.logger.Debug().Msgf("ElectLeaders response: %+v (%+v)", resp, err)
	if err != nil {
		return err
	}
	if err = resp.Error; err != nil {
